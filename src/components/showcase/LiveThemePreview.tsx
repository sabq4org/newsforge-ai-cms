...
const PreviewComponent: React.FC<PreviewComponentProps> = ({ theme }) => {
  const applyThemeStyles = (colors: ThemePreset['colors']) => {
    return {
      ...
      '--secondary-foreground': colors?.secondaryForeground || '#ffffff',
      '--accent': colors?.accent || '#999999',
      '--accent-foreground': colors?.accentForeground || '#ffffff',
      '--muted': colors?.muted || '#f0f0f0',
      '--muted-foreground': colors?.mutedForeground || '#666666',
      '--border': colors?.border || '#e0e0e0',
      '--destructive': colors?.destructive || '#ff0000',
      '--destructive-foreground': colors?.destructiveForeground || '#ffffff',
